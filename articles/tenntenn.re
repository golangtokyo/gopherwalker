...
また、Go Fridayは社内勉強会にはめずらしく@<fn>{tenntenn_fn1}継続して開催されています。
多くの場合、社内勉強会が通常業務に忙殺され長続きしないことは、読者のみなさんもおそらく経験されてるでしょう。
しかし、Go Fridayは執筆時点（2017年10月）で1年以上毎週ほとんど休むことなく続いており、回数は60回を超えています。
そこで本章では、社内勉強会を長く続く意味のあるものにするためのコツも紹介します。

//footnote[tenntenn_fn1][読者のみなさんも社内勉強会が続かなかった経験がきっとあるでしょう]
...
Go Fridayという名前ですが、Go以外も扱っており、@<kw>{Google Cloud Platform,GCP}についても、しばしば話題にあがります。
...
ここでは具体的にどのようにこのポリシーを元に社内勉強会を行っているのか説明します。

=== 長く続けるためには"頑張らない"

社内勉強会が続かない大きな理由として、参加することが負担になるという点があるでしょう。
勉強会に参加するエンジニアは、開発業務を抱えているため、
発表資料を作るという負担があるとなかなか参加しづらいものです。

資料を作らなくとも、社内であればプロジェクトのコードを直接見たり、
参考にしたブログやドキュメントのリンクを社内チャットやWikiに貼り付けるだけでも十分です。
必要があれば、ホワイトボードに図を書いて説明することもできるでしょう。

発表資料を用意したからといって、議論が活発になるわけでもありません。
...
エンジニア、特に担当領域が同じ、サーバサイドエンジニアが集まれば、
たとえ雑談だとしても、それはきっとGoやGCPなど、技術的な話題になることが多いでしょう。
また、プロジェクトをまたぐと、同じサーバサイドエンジニアであっても普段はそこまで議論する機会は無いでしょう。

ソウゾウも「ありそうでなかったをソウゾウする」というミッションを掲げ、
...
@<code>{switch}は定数を分岐するためによく使われる構文です。
...
Goの@<code>{switch-case}は明示的に@<code>{break}を指定しなくても、
...
何かのステータスなどを管理する際に、たとえば@<list>{tenntenn_list2}のように定数を用いることが多いでしょう。
@<code>{type}で新しい型@<code>{Status}を作り、その定数として@<code>{StatusA}と@<code>{StatusB}を定義しています。
...
これらの2つの定数の値は、値として意味を持っているため、
@<code>{iota}を使わずに、値を直接指定しています。
...
また、その定数の値をデータベースに保存する必要がある場合はなおさら@<code>{iota}は使うべきではないでしょう。
...
実際にGo Playground@<fn>{tenntenn_fn2}で動してみると、コンパイルエラーが発生することが分かるでしょう。

//footnote[tenntenn_fn2][@<href>{https://play.golang.org/p/RafJs_O9y8}]

このように、Goの@<code>{switch}では、@<code>{case}の値が重複していると、
コンパイルエラーになります。
新しく定数を追加する際に、前述のようなミスはよく発生するでしょう。
そうした時にコンパイルエラーになると、予め間違いに気づけるため非常に助かります。
そういった意味でも積極的に@<code>{switch}を使うと良いでしょう。
...
サブ機能は@<list>{tenntenn_list5}のように、1つのテスト関数内で、サブテストを複数実行できる機能です。
...
テストケースを構造体のスライスで並べ、そのテストケースに対して、
@<code>{for}で繰り返して、各テストケースをテストするというものです。
...
テーブル駆動テスト利点は、1度テストを書くと、簡単にテストケースを追加することができ、
大量のテストケースを簡単に書くことができます。
...
たとえば、数十から数百からのテストケースは容易に考えられるでしょう。
...
@<list>{tenntenn_list7}のようにテストケース名を出力すれば多少分かりやすくなるでしょう。
...
できれば該当の部分だけを実行したいでしょうが、@<list>{tenntenn_list7}のコードでは、
...
テストを書く際に、よく使う処理を関数にまとめたくなるでしょう。
そうした関数をここではテストヘルパーと呼ぶことにします。
そして、テストヘルパーは次の2つを満たすように作るとよいでしょう。
...
通常であれば、適切にエラー処理し、必要があれば戻り値として呼び出し元に返すでしょう。
しかし、テストヘルパーでエラーを返してしまうと、各テスト関数内で毎回エラー処理をする必要があります。
また、テストヘルパー内で発生するエラーは、ほとんどが回復不可能なエラーです。
そのため、エラーを返すのではなく、@<code>{t.Fatal}メソッドでテストを落としてやる方がよいでしょう。
...
そのような場合は、エラーを値で比較するのではなく、インタフェースを用いると良いでしょう。
...
エラーが起きた場合にSlackに通知されるのは、イチは早くエラーに気づけるため、非常に便利です。
しかし、一方で通知が多すぎると逆に重要なエラーがなんなのか分かりづらくなります。
...
本章では、筆者がおこなっているGo Fridayについてとそこで扱った、
特に面白い話題についてご紹介しました。
